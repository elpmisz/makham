...
if ($action === "create") {
  try {
    ...
    if (intval($count) > 0) {
      $VALIDATION->alert("danger", "ข้อมูลซ้ำในระบบ!", "/location");
    }

    $LOCATION->location_insert([$name, $text]);
...
if ($action === "upload") {
  try {
    ...
    if (!in_array($file_extension, $file_allow)) :
      $VALIDATION->alert("danger", "เฉพาะเอกสาร XLS XLSX CSV!", "/location");
    endif;
    ...
    foreach ($data as $key => $value) {
      if (!in_array($key, [0])) {
        ...
        $count = $LOCATION->uuid_count([$uuid]);
