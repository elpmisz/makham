...
class Location
{
  ...
  {
    $sql = "SELECT COUNT(*) FROM inventory.location WHERE name = ? AND status = 1";
  ...
  {
    $sql = "INSERT INTO inventory.location(uuid,name,text) VALUES(uuid(),?,?)";
    $stmt = $this->dbcon->prepare($sql);
  ...
  {
    $sql = "UPDATE inventory.location SET
    name = ?,
    text = ?,
    status = ?,
    updated = NOW()
  ...
  {
    $sql = "SELECT COUNT(*) FROM inventory.location WHERE uuid = ?";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute($data);
    return $stmt->fetchColumn();
  }
  ...
  {
    $sql = "SELECT COUNT(*) FROM inventory.location";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute();
    ...
    foreach ($result as $row) {
      $status = "<a href='/location/edit/{$row['uuid']}' class='badge badge-{$row['status_color']} font-weight-light'>{$row['status_name']}</a>";
      $data[] = [
        $status,
        $row['name'],
        str_replace("\n", "<br>", $row['text']),
