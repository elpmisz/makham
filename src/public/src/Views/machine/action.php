...
if ($action === "create") {
  try {
    ...
    $count = $MACHINE->machine_count([$name]);
    if (intval($count) > 0) {
      $VALIDATION->alert("danger", "ข้อมูลซ้ำในระบบ!", "/machine");
    }

    $MACHINE->machine_insert([$name, $text]);
    $VALIDATION->alert("success", "ดำเนินการเรียบร้อย!", "/machine");
...
if ($action === "update") {
  try {
    ...
    $MACHINE->machine_update([$name, $text, $status, $uuid]);
    $VALIDATION->alert("success", "ดำเนินการเรียบร้อย!", "/machine");
  } catch (PDOException $e) {
    die($e->getMessage());
  }
...
if ($action === "upload") {
  try {
    ...
    if (!in_array($file_extension, $file_allow)) :
      $VALIDATION->alert("danger", "เฉพาะเอกสาร XLS XLSX CSV!", "/machine");
    endif;
