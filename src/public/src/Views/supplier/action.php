...
if ($action === "create") {
  try {
    ...
    $count = $SUPPLIER->supplier_count([$name]);
    if (intval($count) > 0) {
      $VALIDATION->alert("danger", "ข้อมูลซ้ำในระบบ!", "/supplier");
    }

    $SUPPLIER->supplier_insert([$name, $vat, $email, $contact, $address, $sub, $text]);
...
if ($action === "upload") {
  try {
    ...
    if (!in_array($file_extension, $file_allow)) :
      $VALIDATION->alert("danger", "เฉพาะเอกสาร XLS XLSX CSV!", "/supplier");
    endif;
    ...
    foreach ($data as $key => $value) {
      if (!in_array($key, [0])) {
        ...
        $count = $SUPPLIER->uuid_count([$uuid]);

        $row = $SUPPLIER->subcode([$sub]);
        $subcode = $row['subcode'];
        if (intval($count) > 0) {
          $SUPPLIER->supplier_update([$name, $vat, $email, $contact, $address, $subcode, $text, $status, $uuid]);
        } else {
          $SUPPLIER->supplier_insert([$name, $vat, $email, $contact, $address, $subcode, $text]);
        }
      }
    }
...
if ($action === "address-select") {
  try {
    ...
    $result = $SUPPLIER->address_select($keyword);
...
if ($action === "address-view") {
  try {
    ...
    $sub = $data['sub'];
    $result = $SUPPLIER->address_view([$sub]);
