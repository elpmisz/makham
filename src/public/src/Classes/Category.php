...
class Category
{
  ...
  public function category_count($data)
  {
    $sql = "SELECT COUNT(*) FROM inventory.category WHERE name = ? AND status = 1";
  ...
  {
    $sql = "INSERT INTO inventory.category(uuid,name,text) VALUES(uuid(),?,?)";
    $stmt = $this->dbcon->prepare($sql);
  ...
  {
    $sql = "UPDATE inventory.category SET
    name = ?,
    text = ?,
    status = ?,
    updated = NOW()
  ...
  {
    $sql = "SELECT COUNT(*) FROM inventory.category WHERE uuid = ?";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute($data);
    return $stmt->fetchColumn();
  }
  ...
  {
    $sql = "SELECT COUNT(*) FROM inventory.category";
    $stmt = $this->dbcon->prepare($sql);
    $stmt->execute();
    ...
    foreach ($result as $row) {
      $status = "<a href='/category/edit/{$row['uuid']}' class='badge badge-{$row['status_color']} font-weight-light'>{$row['status_name']}</a>";
      $data[] = [
        $status,
        $row['name'],
        str_replace("\n", "<br>", $row['text']),
