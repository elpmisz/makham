...
if ($action === "create") {
  try {
    ...
    if (intval($count) > 0) {
      $VALIDATION->alert("danger", "ข้อมูลซ้ำในระบบ!", "/brand");
    }

    $BRAND->brand_insert([$name, $text]);
    $VALIDATION->alert("success", "ดำเนินการเรียบร้อย!", "/brand");
...
if ($action === "update") {
  try {
    ...
    $BRAND->brand_update([$name, $text, $status, $uuid]);
    $VALIDATION->alert("success", "ดำเนินการเรียบร้อย!", "/brand");
  } catch (PDOException $e) {
    die($e->getMessage());
  }
...
if ($action === "upload") {
  try {
    ...
    if (!in_array($file_extension, $file_allow)) :
      $VALIDATION->alert("danger", "เฉพาะเอกสาร XLS XLSX CSV!", "/brand");
    endif;
